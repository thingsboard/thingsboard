...
@Slf4j
public class AttributesUpdateNonObserveClient {

    private CoapClient client;
    private ExecutorService executor = Executors.newFixedThreadPool(1);
    private final CountDownLatch responsesLatch = new CountDownLatch(2);

    private AttributesUpdateNonObserveClient(String host, int port, String token) {
    ...
    public static void main(String args[]) {
        if (args.length != 3) {
            log.error("Usage: java -jar {}.jar host port device_token", AttributesUpdateNonObserveClient.class.getSimpleName());
        }
        final AttributesUpdateNonObserveClient emulator = new AttributesUpdateNonObserveClient(args[0], Integer.parseInt(args[1]), args[2]);
        emulator.start();
