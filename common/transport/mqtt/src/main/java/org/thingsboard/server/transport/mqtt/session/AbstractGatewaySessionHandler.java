...
/**
 * Created by nickAS21 on 23.12.22
 */
...
public abstract class AbstractGatewaySessionHandler {
    ...
    protected static final String CAN_T_PARSE_VALUE = "Can't parse value: ";
    private static final String DEVICE_PROPERTY = "device";

    protected final MqttTransportContext context;
    private final TransportService transportService;
    protected final TransportDeviceInfo transportDeviceInfo;
    protected final UUID sessionId;
    private final ConcurrentMap<String, Lock> deviceCreationLockMap;
    private final ConcurrentMap<String, MqttDeviceAwareSessionContext> devices = new ConcurrentHashMap<>();
    private final ConcurrentMap<String, ListenableFuture<MqttDeviceAwareSessionContext>> deviceFutures = new ConcurrentHashMap<>();
    ...
    public AbstractGatewaySessionHandler(DeviceSessionCtx deviceSessionCtx, UUID sessionId) {
        ...
        this.transportDeviceInfo = deviceSessionCtx.getDeviceInfo();
    ...
    ConcurrentReferenceHashMap<String, Lock> createWeakMap() {
        return new ConcurrentReferenceHashMap<>(16, WEAK);
    }
    ...
    void deregisterSession(String deviceName) {
        ...
        if (deviceSessionCtx != null) {
            ...
            log.debug("[{}] Device [{}] was already removed from the gateway or sparkplugNode session", sessionId, deviceName);
        }
    }
    ...
    private ListenableFuture<MqttDeviceAwareSessionContext> getDeviceCreationFuture(String deviceName, String deviceType) {
        ...
        try {
            transportService.process(GetOrCreateDeviceFromGatewayRequestMsg.newBuilder()
                            ...
                            .setGatewayIdMSB(transportDeviceInfo.getDeviceId().getId().getMostSignificantBits())
                            .setGatewayIdLSB(transportDeviceInfo.getDeviceId().getId().getLeastSignificantBits())
                            .setSparkplug(this.deviceSessionCtx.isSparkplug())
                            .build(),
                    new TransportServiceCallback<>() {
                        @Override
                        public void onSuccess(GetOrCreateDeviceFromGatewayResponse msg) {
                             GatewayDeviceSessionCtx deviceSessionCtx = new GatewayDeviceSessionCtx(AbstractGatewaySessionHandler.this, msg.getDeviceInfo(), msg.getDeviceProfile(), mqttQoSMap, transportService);
    ...
    protected void onDeviceConnectProto(MqttPublishMessage mqttMsg) throws AdaptorException {
        try {
            ...
            String deviceType = StringUtils.isEmpty(transportDeviceInfo.getDeviceType()) ? DEFAULT_DEVICE_TYPE : transportDeviceInfo.getDeviceType();
            processOnConnect(mqttMsg, deviceName, deviceType);
        } catch (Exception e) {
    ...
    protected void onDeviceTelemetryProto(int msgId, ByteBuf payload) throws AdaptorException {
        try {
            ...
            if (!CollectionUtils.isEmpty(deviceMsgList)) {
                            ...
                            new FutureCallback<>() {
                                ...
                                public void onSuccess(@Nullable MqttDeviceAwareSessionContext deviceCtx) {
                                    ...
                                    try {
                                        ...
                                        log.warn("[{}][{}] Failed to convert telemetry: {}", transportDeviceInfo.getDeviceId(), deviceName, msg, e);
                                        channel.close();
                                    }
                                }
                ...
                log.debug("[{}] Devices telemetry messages is empty for: [{}]", sessionId, transportDeviceInfo.getDeviceId());
                throw new IllegalArgumentException("[" + sessionId + "] Devices telemetry messages is empty for [" + transportDeviceInfo.getDeviceId() + "]");
            }
        } catch (RuntimeException | InvalidProtocolBufferException e) {
            throw new AdaptorException(e);
        }
                        ...
                        new FutureCallback<>() {
                            ...
                            public void onSuccess(@Nullable MqttDeviceAwareSessionContext deviceCtx) {
                                ...
                                try {
                                    ...
                                    log.warn("[{}][{}] Failed to convert claim message: {}", transportDeviceInfo.getDeviceId(), deviceName, deviceEntry.getValue(), e);
                                }
                            }
    ...
    private void onDeviceClaimProto(int msgId, ByteBuf payload) throws AdaptorException {
        try {
            ...
            if (!CollectionUtils.isEmpty(claimMsgList)) {
                            ...
                            new FutureCallback<>() {
                                ...
                                public void onSuccess(@Nullable MqttDeviceAwareSessionContext deviceCtx) {
                                    ...
                                    try {
                                        ...
                                        log.warn("[{}][{}] Failed to convert claim message: {}", transportDeviceInfo.getDeviceId(), deviceName, claimRequest, e);
                                    }
                                }
                ...
                log.debug("[{}] Devices claim messages is empty for: [{}]", sessionId, transportDeviceInfo.getDeviceId());
                throw new IllegalArgumentException("[" + sessionId + "] Devices claim messages is empty for [" + transportDeviceInfo.getDeviceId() + "]");
            }
        } catch (RuntimeException | InvalidProtocolBufferException e) {
            throw new AdaptorException(e);
        }
    ...
    private void onDeviceAttributesProto(int msgId, ByteBuf payload) throws AdaptorException {
        try {
            ...
            if (!CollectionUtils.isEmpty(attributesMsgList)) {
                            ...
                            new FutureCallback<>() {
                                ...
                                public void onSuccess(@Nullable MqttDeviceAwareSessionContext deviceCtx) {
                                    ...
                                    try {
                                        ...
                                        log.warn("[{}][{}] Failed to process device attributes command: {}", transportDeviceInfo.getDeviceId(), deviceName, kvListProto, e);
                                    }
                                }
                ...
                log.debug("[{}] Devices attributes keys list is empty for: [{}]", sessionId, transportDeviceInfo.getDeviceId());
                throw new IllegalArgumentException("[" + sessionId + "] Devices attributes keys list is empty for [" + transportDeviceInfo.getDeviceId() + "]");
            }
        } catch (RuntimeException | InvalidProtocolBufferException e) {
            throw new AdaptorException(e);
        }
