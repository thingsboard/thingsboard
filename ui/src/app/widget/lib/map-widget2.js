...
export default class TbMapWidgetV2 {
	...
	constructor(mapProvider, drawRoutes, ctx, useDynamicLocations, $element, isEdit) {
		...
		var onMapClick = function (event, latlng) {
			var descriptors = tbMap.ctx.actionsApi.getActionDescriptors('mapClick');
			if (descriptors) {
				var entityInfo = tbMap.ctx.actionsApi.getActiveEntityInfo();
				var entityId = entityInfo ? entityInfo.entityId : null;
				var entityName = entityInfo ? entityInfo.entityName : null;
				tbMap.ctx.actionsApi.handleWidgetAction(event, descriptors[0], entityId, entityName, latlng);
			}
		}

    let openStreetMapProvider = {};
		if (mapProvider === 'google-map') {
			this.map = new TbGoogleMap($element, this.utils, initCallback, this.defaultZoomLevel, this.dontFitMapBounds, settings.disableScrollZooming, minZoomLevel, settings.gmApiKey, settings.gmDefaultMapType, settings.defaultCenterPosition, markerClusteringSetting, onMapClick);
			...
			this.map = new TbOpenStreetMap($element, this.utils, initCallback, this.defaultZoomLevel, this.dontFitMapBounds, settings.disableScrollZooming, minZoomLevel, openStreetMapProvider, null,settings.defaultCenterPosition, markerClusteringSetting, onMapClick);
		} else if (mapProvider === 'here') {
			openStreetMapProvider.name = settings.mapProvider;
			this.map = new TbOpenStreetMap($element, this.utils, initCallback, this.defaultZoomLevel, this.dontFitMapBounds, settings.disableScrollZooming, minZoomLevel, openStreetMapProvider, settings.credentials, settings.defaultCenterPosition, markerClusteringSetting, onMapClick);
			...
			this.map = new TbTencentMap($element, this.utils, initCallback, this.defaultZoomLevel, this.dontFitMapBounds, settings.disableScrollZooming, minZoomLevel, settings.tmApiKey, settings.tmDefaultMapType, settings.defaultCenterPosition, markerClusteringSetting, onMapClick);
		}
