...
public interface DeviceRepository extends JpaRepository<DeviceEntity, UUID>, ExportableEntityRepository<DeviceEntity> {
    ...
    @Query("SELECT d FROM DeviceEntity d WHERE d.tenantId = :tenantId " +
            ...
            "AND (:textSearch IS NULL OR ilike(d.name, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true " +
            "OR ilike(d.label, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true)")
    ...
    @Query("SELECT d FROM DeviceInfoEntity d " +
            "WHERE d.tenantId = :tenantId " +
            "AND (:customerId IS NULL OR d.customerId = CAST(COALESCE(CAST(:customerId as text), '') as uuid)) " +
            "AND (:edgeId IS NULL OR d.id IN (SELECT re.toId FROM RelationEntity re WHERE re.toType = 'DEVICE' AND re.relationTypeGroup = 'EDGE' AND re.relationType = 'Contains' AND re.fromType = 'EDGE' AND re.fromId = CAST(COALESCE(CAST(:edgeId as text), '') as uuid))) " +
            "AND ((:deviceType) IS NULL OR d.type = :deviceType) " +
            "AND (:deviceProfileId IS NULL OR d.deviceProfileId = CAST(COALESCE(CAST(:deviceProfileId as text), '') as uuid)) " +
            "AND ((:filterByActive) = FALSE OR d.active = :deviceActive) " +
            "AND (:textSearch IS NULL OR ilike(d.name, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true " +
            "OR ilike(d.label, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true " +
            "OR ilike(d.type, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true " +
            "OR ilike(d.customerTitle, CONCAT('%', COALESCE(CAST(:textSearch as text), ''), '%')) = true)")
