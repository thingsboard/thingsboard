...
@Table(name = ModelConstants.RULE_CHAIN_INFO_VIEW_NAME)
public class RuleChainInfoEntity extends BaseSqlEntity<RuleChainInfo> implements SearchTextEntity<RuleChainInfo> {
    ...
    @Column(name = ModelConstants.RULE_CHAIN_ERROR_PRESENT_PROPERTY)
    private boolean errorPresent;

    public RuleChainInfoEntity() {
    }

    public RuleChainInfoEntity(RuleChainEntity ruleChain, boolean errorPresent) {
        if (ruleChain.getId() != null) {
            this.setUuid(ruleChain.getUuid());
        }
        ...
        if (ruleChain.getExternalId() != null) {
            this.externalId = ruleChain.getExternalId();
        }
    ...
    @Override
    public RuleChainInfo toData() {
        RuleChainInfo ruleChain = new RuleChainInfo(new RuleChainId(this.getUuid()));
        ...
        ruleChain.setErrorPresent(errorPresent);
        return ruleChain;
    }
}
