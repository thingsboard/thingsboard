...
public class GeofencingProcessor {
    ...
    public GeofenceResponse process(Long ts, EntityId entityId, List<EntityId> matchedGeofences) throws ExecutionException, InterruptedException {
        List<EntityId> enteredGeofences = emptyList();
        List<EntityId> leftGeofences = emptyList();
        List<EntityId> insideGeofences = emptyList();

        Set<GeofenceState> geofenceStates = fetchGeofenceStatesForEntity(entityId);
        ...
        persistState(entityId, geofenceStates);
    ...
    private void persistState(EntityId entityId, Set<GeofenceState> geofenceStates) {
        ObjectNode jsonNode = JacksonUtil.newObjectNode();

        jsonNode.set(GEOFENCE_STATES_KEY, JacksonUtil.valueToTree(geofenceStates));

        BaseAttributeKvEntry attributeKvEntry = new BaseAttributeKvEntry(new JsonDataEntry(nodeConfiguration.getGeofenceStateAttributeKey(), jsonNode.toString()), System.currentTimeMillis());
    ...
    public Set<GeofenceState> fetchGeofenceStatesForEntity(EntityId entityId) throws ExecutionException, InterruptedException {
        Optional<AttributeKvEntry> attributeKvEntry = context.getAttributesService().find(context.getTenantId(), entityId, AttributeScope.SERVER_SCOPE, nodeConfiguration.getGeofenceStateAttributeKey()).get();
