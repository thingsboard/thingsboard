...
public class TbGetCustomerDetailsNode extends TbAbstractGetEntityDetailsNode<TbGetCustomerDetailsNodeConfiguration> {
        ...
        switch (msg.getOriginator().getEntityType()) {
            case DEVICE:
                ...
                if (!device.getCustomerId().isNullUid()) {
                    ...
                    throw new RuntimeException("Device with name '" + device.getName() + "' doesn't assign to Customer.");
                }
            case ASSET:
                ...
                if (!asset.getCustomerId().isNullUid()) {
                    ...
                    throw new RuntimeException("Asset with name '" + asset.getName() + "' doesn't assign to Customer.");
                }
            case ENTITY_VIEW:
                ...
                if (!entityView.getCustomerId().isNullUid()) {
                    ...
                    throw new RuntimeException("EntityView with name '" + entityView.getName() + "' doesn't assign to Customer.");
                }
            default:
                throw new RuntimeException("Entity with entityType '" + msg.getOriginator().getEntityType() + "' doesn't support Customer assigning.");
        }
    }
    ...
    private JsonElement addCustomerProperties(JsonElement data, Customer customer, EntityDetails entityDetails) {
        ...
        switch (entityDetails) {
            case ADDRESS:
                if (customer.getAddress() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "address", customer.getAddress());
                break;
            case ADDRESS2:
                if (customer.getAddress2() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "address2", customer.getAddress2());
                break;
            case CITY:
                if (customer.getCity() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "city", customer.getCity());
                break;
            case COUNTRY:
                if (customer.getCountry() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "country", customer.getCountry());
                break;
            case STATE:
                if (customer.getState() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "state", customer.getState());
                break;
            case EMAIL:
                if (customer.getEmail() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "email", customer.getEmail());
                break;
            case PHONE:
                if (customer.getPhone() != null) dataAsObject.addProperty(CUSTOMER_PREFIX + "phone", customer.getPhone());
                break;
            case ZIP:
