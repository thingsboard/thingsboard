...
class AlarmRuleState {
    ...
    public boolean validateAttrUpdate(Set<EntityKey> changedKeys) {
        ...
        for (EntityKey key : changedKeys) {
            ...
            if (EntityKeyType.CLIENT_ATTRIBUTE.equals(keyType) || EntityKeyType.SERVER_ATTRIBUTE.equals(keyType) || EntityKeyType.SHARED_ATTRIBUTE.equals(keyType)) {
                key = new EntityKey(EntityKeyType.ATTRIBUTE, key.getKey(), true);
            }
            if (entityKeys.contains(key)) {
                return true;
            }
    ...
    private <T> EntityKeyValue getDynamicPredicateValue(DataSnapshot data, FilterPredicateValue<T> value) {
        ...
        if (value.getDynamicValue() != null) {
            ekv = data.getValue(new EntityKey(EntityKeyType.ATTRIBUTE, value.getDynamicValue().getSourceAttribute(), true));
            if (ekv == null) {
                ekv = data.getValue(new EntityKey(EntityKeyType.SERVER_ATTRIBUTE, value.getDynamicValue().getSourceAttribute(), true));
                if (ekv == null) {
                    ekv = data.getValue(new EntityKey(EntityKeyType.SHARED_ATTRIBUTE, value.getDynamicValue().getSourceAttribute(), true));
                    if (ekv == null) {
                        ekv = data.getValue(new EntityKey(EntityKeyType.CLIENT_ATTRIBUTE, value.getDynamicValue().getSourceAttribute(), true));
                    }
                }
            }
        }
