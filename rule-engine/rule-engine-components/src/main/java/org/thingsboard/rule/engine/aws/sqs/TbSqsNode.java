...
@RuleNode(
        ...
        nodeDescription = "发布消息到AWS SQS",
        nodeDetails = "该节点将消息的有效负载和元数据属性发布到AWS SQS队列。出站消息将包含来自AWS SQS的消息元数据" +
                "的响应字段(<code>messageId</code>, <code>requestId</code>, <code>messageBodyMd5</code>, <code>messageAttributesMd5</code>" +
                ", <code>sequenceNumber</code>)。" +
                "例如，可以使用<code>metadata.requestId</code>来访问<b>requestId</b>字段。",
