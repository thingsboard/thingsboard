...
public class TbMqttNode extends TbAbstractExternalNode {
    ...
    private String getClientId(TbContext ctx) throws TbNodeException {
        ...
        int maxLength = mqttNodeConfiguration.getProtocolVersion() == MqttVersion.MQTT_3_1 ? 23 : 256;
        if (clientId.length() > maxLength) {
            String clientIdPreview = clientId.substring(0, maxLength) + "...";
            throw new TbNodeException("Client ID is too long '" + clientIdPreview + "'. " +
                    "The length of Client ID cannot be longer than " + maxLength + ", but current length is " + clientId.length() + ".", true);
        }
        return clientId;
    }
