...
public abstract class BaseOtaPackageControllerTest extends AbstractControllerTest {
    ...
    private static final String URL = "https://test.com/testurl";
    ...
    @Test
    public void testSaveOtaPackageUsingUrl() throws Exception {
        OtaPackageInfo firmwareInfo = new OtaPackageInfo();
        firmwareInfo.setDeviceProfileId(deviceProfileId);
        firmwareInfo.setType(FIRMWARE);
        firmwareInfo.setTitle(TITLE);
        ...
        OtaPackageInfo savedFirmwareInfo = save(firmwareInfo, true);
        Assert.assertEquals(savedFirmwareInfo.getUrl(), URL);
    }

    @Test
    public void testSaveOtaPackageBlankUrlValidation() {
        OtaPackageInfo firmwareInfo = new OtaPackageInfo();
        firmwareInfo.setDeviceProfileId(deviceProfileId);
        firmwareInfo.setType(FIRMWARE);
        firmwareInfo.setTitle(TITLE);
        ...
        firmwareInfo.setUrl("   ");
        try {
            save(firmwareInfo, true, status().isBadRequest());
        } catch (Exception e) {
            Assert.assertEquals(e.getMessage(), "Ota package URL should be specified!");
        }
    ...
    @Test
    public void testSaveOtaPackageCantUpdateURL() throws Exception {
        OtaPackageInfo firmwareInfo = new OtaPackageInfo();
        firmwareInfo.setDeviceProfileId(deviceProfileId);
        firmwareInfo.setType(FIRMWARE);
        firmwareInfo.setTitle(TITLE);
        ...
        try {
            savedFirmwareInfo.setUrl("newURL");
            save(savedFirmwareInfo, true, status().isBadRequest());
        } catch (Exception e) {
            Assert.assertEquals(e.getMessage(), "Updating otaPackage URL is prohibited!");
        }
    }

    private OtaPackageInfo save(OtaPackageInfo firmwareInfo, boolean isUrl) throws Exception {
        return save(firmwareInfo, isUrl, status().isOk());
    }

    private OtaPackageInfo save(OtaPackageInfo firmwareInfo, boolean isUrl, ResultMatcher resultMatcher) throws Exception {
        return doPost("/api/otaPackage?isUrl=" + isUrl, firmwareInfo, OtaPackageInfo.class, resultMatcher);
    }
