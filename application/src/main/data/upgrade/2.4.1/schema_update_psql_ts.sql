...
BEGIN
  ...
  IF valid_version = FALSE THEN
    RAISE 'Postgres version should be at least more than 10!';
  ELSE
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE 'CREATE TABLE IF NOT EXISTS ts_kv_' || partition_date ||
            ' PARTITION OF ts_kv_partitioned(PRIMARY KEY (entity_id, key, ts)) FOR VALUES FROM (' || from_ts ||
            ') TO (' || to_ts || ');';
