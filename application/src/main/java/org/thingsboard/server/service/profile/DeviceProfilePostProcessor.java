...
public class DeviceProfilePostProcessor implements EntityProfilePostProcessor<DeviceProfile> {
    ...
    @Override
    public Class<DeviceProfile> getProfileClass() {
        return DeviceProfile.class;
    }

    @Override
    public void setDefaultValues(DeviceProfile profile) {
        if (profile.getInactivityTimeout() == null) {
            profile.setInactivityTimeout(TimeUnit.SECONDS.toMillis(defaultInactivityTimeoutInSec));
        }
    }
}
